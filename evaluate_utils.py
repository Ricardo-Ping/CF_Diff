    ...
    for index in range(len(topN)):
        ...
        for i in range(len(predictedIndices)):
            if len(GroundTruth[i]) != 0:
                ...
                for j in range(min(topN[index], len(predictedIndices[i]))):
                    if predictedIndices[i][j] in GroundTruth[i]:
                        # Recall 和 Precision 相关
                        userHit += 1
                ...
                # 计算单个用户的Precision, Recall, NDCG, MRR
        ...
        # 对所有用户的平均值
        precision.append(round(sumForPrecision / len(predictedIndices), 5))
        recall.append(round(sumForRecall / len(predictedIndices), 5))
        NDCG.append(round(sumForNdcg / len(predictedIndices), 5))
        MRR.append(round(sumForMRR / len(predictedIndices), 5))
